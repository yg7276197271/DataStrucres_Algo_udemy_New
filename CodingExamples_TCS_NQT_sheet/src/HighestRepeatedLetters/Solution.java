package HighestRepeatedLetters;
...
class Solution {
	static void HighestRepeatedLetters(String str) {
		...
		for (int left = 0; left < len;) {
			...
			while (right < len && str.charAt(right) != ' ') {
				right++;
			} //To get the first word ,second word , 3rd etc
			...
			for (int index = 0; index < 26; index++) {
				if (frequency[index] > 1) {
					curr_maximumword = Integer.max(curr_maximumword, frequency[index]);
				}
			}
	...
	public static void main(String args[]) {
		...
		String str = "google gooogle microsoft";
